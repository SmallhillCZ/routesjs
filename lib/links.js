...
class RoutesLinks {
    ...
    static root(req) {
        ...
        for (let route of routes_store_1.routesStore.routes) {
            ...
            link.allowed[method] = acl_1.RoutesACL.canRoute(route, req);
        }
        return links;
    }
    static assignLinks(docs, routes, req) {
        ...
        for (let doc of arrayDocs) {
            ...
            for (let route of routes) {
                if (route.hideDocs)
                    continue;
                if (route.queryParsed && !route.queryParsed.matches(doc, false))
                    continue;
                // check if ACL doc allowed
                const allowed = acl_1.RoutesACL.canRouteDoc(route, doc, req);

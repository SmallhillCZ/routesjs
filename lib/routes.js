...
class Routes {
    ...
    child(path, childRoutes) {
        ...
        childRoutes.rootUrl = functions_1.pathToTemplate(path);
    ...
    static findRoute(resource, link, method) {
        return routes_store_1.routesStore.routes.find(route => route.resource === resource && route.link === link && (method.toLowerCase() === "action" ? route.action === true : route.method === "action"));
    }
    static setACL(acl) {
        if (!acl.permissions)
